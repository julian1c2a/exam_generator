...
\begin{center} \begin{circuitikz}[scale=1.2, transform shape]
	\draw (0,0) node[flipflop T, external pins width=0](FF1){Q0}
	      (5,0) node[flipflop T, external pins width=0](FF2){Q1};
	% Clock Bus
	% Clock Bus
	\draw (FF1.pin 3) ++(0,0) -- ++(-0.5,0) -- ++(0,-2.0) coordinate(clk_bus);
	\draw (FF2.pin 3) ++(0,0) -- ++(-0.5,0) -- (clk_bus -| FF2.pin 3) -- (clk_bus);
